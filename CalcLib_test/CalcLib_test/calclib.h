...
// результат живёт внутри объекта до следующего вызова evaluate
void __cdecl calc_evaluate (HCALC h, const char* expr);
...
CALCAPI HCALC  __cdecl calc_create  (void);
CALCAPI void   __cdecl calc_destroy (HCALC h);
CALCAPI void   __cdecl calc_evaluate(HCALC h, char* expr);
CALCAPI void __cdecl calc_print (HCALC h, char* str, int options, int binw);
CALCAPI void __cdecl calc_print_res (HCALC h, char* str, int options, int binw);
CALCAPI void __cdecl calc_addfn (HCALC h, char* name, void *fn);
