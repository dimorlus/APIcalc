       ...
       if (rt == rtCheb)
        {
         ...
         for (int k = 2; k <= degree; k++) T[k] = 2.0L * x_norm * T[k - 1] - T[k - 2];

         // Build normal equation matrix: M[r][c] = Σ T_r(x) * T_c(x)
         for (int r = 0; r <= degree; r++)
          for (int c = 0; c <= degree; c++) M_cheb[r * (degree + 1) + c] += T[r] * T[c];

         // Build right-hand side: B[r] = Σ y * T_r(x)
         for (int r = 0; r <= degree; r++) B_cheb[r] += y * T[r];
        }
       else
        {
 ...
 if (rt == rtCheb)
  {
   ...
   for (int r = 0; r < dim; r++)
    {
     for (int c = 0; c < dim; c++) valM.mval[r * dim + c] = M_cheb[r * dim + c];
 ...
 else
  {
   ...
   for (int r = 0; r < dim; r++)
    {
     for (int c = 0; c < dim; c++) valM.mval[r * dim + c] = S[r + c];
     valB.mval[r] = SY[r];
    }
  }
 ...
 if (rt == rtCheb)
  {
   ...
   for (int i = 0; i < dim; i++) res_mval[i] = valCoeff.mval[i];
 ...
 else
  {
   ...
   for (int i = 0; i < dim; i++) res_mval[i] = valCoeff.mval[degree - i];

   // Inverse transformation for exponential and power types
   if (rt == rtExp || rt == rtPow) res_mval[dim - 1] = Exp (res_mval[dim - 1]);
  }
