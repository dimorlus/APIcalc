...
- **История вычислений**: Отслеживание предыдущих вычислений с быстрым доступом через выпадающее меню
...
3E                  # То же, что 3 * E ≈ 8.15 (E в верхнем регистре, чтобы избежать экспоненциальной записи)
```
...
- **Используйте верхний регистр для переменных, чтобы избежать конфликтов**:
  ...
  - `2E` → `2 * E` ≈ 5.44 (заглавная `E` как константа, а не экспоненциальная запись)

**Рекомендуемое соглашение об именовании**: Используйте **ВЕРХНИЙ РЕГИСТР** для констант/переменных, чтобы избежать конфликтов с научными суффиксами и мнимой единицей (например, `PI`, `E`, `PHI`, `X`, `Y`, `Z`).

## Горячие клавиши

- **Ctrl+N**: Новое вычисление (очистить выражение)
- **Ctrl+C**: Копировать результат в буфер обмена
- **Ctrl+V**: Вставить из буфера обмена в выражение
- **F1**: Показать содержимое справки

## Пункты меню

### Calc

- **Pas style**: Переключить отображение ввода в стиле pascal
...
Разместите файл CHM справки как `help.chm` в том же каталоге, что и исполняемый файл, чтобы включить функциональность справки по F1.
