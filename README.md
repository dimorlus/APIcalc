...
* **if(cond, true, false)**: If the first operand `cond` is not 0, the second operand `true` returns; otherwise, the third operand `false` returns. The returned operands can be any type, including different types.  
  ...
  {sign(x)if(x>0,1,-1)}
  ```   
* **polynom([(a, b, c)])**: function find roots of polynomial given coefficients in a  row-matrix. Coefficients ordered from highest degree to constant: $$[a_n, a_{n-1}, ..., a_1, a_0]$$   for polynomial: $$a_nx^{n} + a_{n-1}x^{n-1} + ... + a_1*x + a_0 = 0$$. 
