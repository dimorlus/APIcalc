...
echo ```
for %%I in (".\ccalc\bin\Win32\Release\ccalc.exe") do echo %%~nxI - %%~zI bytes
...
echo ```
for %%I in (".\ccalc\bin\x64\Release\ccalc.exe") do echo %%~nxI - %%~zI bytes
...
echo ```
for %%I in (".\gcc_release\ccalc.exe") do echo %%~nxI - %%~zI bytes
...
echo ```
for %%I in (".\gcc_release\calclib.dll") do echo %%~nxI - %%~zI bytes
for %%I in (".\ccalc\bin\x64\Release\dll\ccalc.exe") do echo %%~nxI - %%~zI bytes
...
echo ```
for %%I in (".\BCB\ccalc.exe") do echo %%~nxI - %%~zI bytes
