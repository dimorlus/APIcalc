# ──────────────────────────────────────────────────────────────
# Combined makefile — builds CLI, GUI and DLL from one engine compile
# ──────────────────────────────────────────────────────────────
...
CXXFLAGS += -Os -DNDEBUG -ffunction-sections -fdata-sections \
            -fvisibility=hidden -fno-math-errno -fomit-frame-pointer \
            -fno-ident
...
$(OUTDIR)/%.o: %.cpp gcc_make.mak
	$(CXX) $(CXXFLAGS) -c $< -o $@

# calclib.o needs DLL export defines
$(OUTDIR)/calclib.o: calclib.cpp gcc_make.mak
